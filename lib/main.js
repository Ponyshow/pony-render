...
function Pony(options) {
  ...
  Renderer.paragraph = function(text) {
    ...
    if (self.sections > 0 || !self.isPonyFile) {
      ...
      // Add to heading and ignore all other text
  ...
  Renderer.image = function(href, title, text) {
    ...
    var out = null;
		...
		var attr = parseClass(text);

    var out = '<img src="' + href + '" alt="' + text + '" class="' + attr.cls + '"';
    if (title) {
      out += ' title="' + title + '"';
    }
    if (attr.style) {
      out += ' style="' + attr.style + '"';
    }
  ...
  Renderer.code = function(text,lang) {
		...
		// parse style attr if available
		if (lang.indexOf(']') >= 0) {
			lang = lang.split(']');
			attr = lang[0].replace('[','');
			lang = lang[1];
			attr = parseClass(attr);
		}
    ...
    return out+'<code class="language-'+lang+'">'+text+'</code></pre>';
  }
  ...
  Renderer.table = function(header, body) {
	...
	Renderer.blockquote = function(quote) {
		...
		// parse bracket style attr if available
		if (quote.indexOf('<p>[') >= 0) {			
			var re = /^<p>\[(.*\S+)?\]/gm;
			var m = re.exec(quote);
			attr = parseClass(m[1]);
			
			// remove bracket styling
			quote = quote.replace(m[0],'<p>');
			
		}
		
		var out = '<blockquote data-test="bar" ';
